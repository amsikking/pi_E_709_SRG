...
class Controller:
    '''
    Basic device adaptor for PI E-709.SRG, compact (cost-optimized) single axis
    digital piezo controller for SGS piezoresistive and capacitive sensors.
    ...
    def set_analog_control_limits(
        ...
        assert 0 <= v_min <= 10 and 0 <= v_max <= 10
        ...
        self.gain = 0.05 * (z_max_ai - z_min_ai) / (v_max - v_min)
        self.offset = z_max_ai - self.gain * (20 * v_max - 100) # y = mx + c...
    ...
    def get_voltage_for_move_um(self, z, relative=True):
        ...
        # z = self.offset + self.gain * (20 * z_voltage - 100) (y = mx + c)
        z_voltage = float(((z - self.offset) / self.gain + 100) / 20)
...
if __name__ == '__main__':
    piezo = Controller('COM10', 0, 500, verbose=True, very_verbose=False)
...
##    piezo._set_parameter('0x07000901', 0.00000000)
    ...
    piezo.move_um(100, relative=False)              # move somewhere
    piezo.set_analog_control_limits(0, 10, 0, 500)  # configure analog
